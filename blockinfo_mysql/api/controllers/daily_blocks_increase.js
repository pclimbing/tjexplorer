...
function daily_blocks_increase(req, res) {
  ...
  //onnection.connect();
  connection.query('SELECT day , AVG(number) AS avgHeight FROM ( SELECT number, LEFT(timestamp,10) AS day, number AS number FROM transactions) as daytable group by day',
