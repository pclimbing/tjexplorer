...
var Demo = function() {
    ...
    Basic = function(a) {
        this.elem = a;
        this.data = function() {
            var op = {tooltip:{formatter:"{a} <br/>{b} : {c}"},toolbox:{feature:{restore:{},saveAsImage:{}}},series:[{name:'交易tps',type:'gauge',detail:{formatter:"{value}",width:"90",height:"60"},center:["50%","70%"],radius:"120%",min:0,max:220,startAngle:180,endAngle:0,data:[{value:50,name:'交易tps(交易数/时)'}]}]}
            return op
            // return '{"tooltip":{"formatter":"{a} <br/>{b} : {c}"},"series":[{"name":"tps","type":"gauge","detail":{"formatter":"{value}","width":"90","height":"60"},"center":["50%","70%"],"radius":"120%","startAngle":180,"endAngle":0,"data":[{"value":50,"name":"tps"}]}]}'
        };
    ...
    Block = function(a) {
        this.elem = a;
        this.data = function() {
            var op ={title:{text:'区块高度'},legend:{data:['bar'],align:'left'},color:['#3398DB'],tooltip:{trigger:'axis',axisPointer:{type:'line'}},tooltip:{},xAxis:{data:[]},yAxis:{},series:[{name:'日期',type:'bar',data:[]}]}
            return op
        };
        this.render = function() {
            $("#" + this.elem).height(400);
            ...
            chart.setOption(this.data())
                ...
                $.get("http://10.15.190.230:10010/daily_blocks_height",function(data,status){
                    ...
                    for(var j =0; j<a.length; j++){
                        ...
                        seriesdata.push(Math.round(tmp[1],0))
                    }
